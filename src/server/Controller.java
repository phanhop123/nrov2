...
//                Logger.logException(Controller.class, e);
                        ...
                        if (player != null && player.isPl()) {
                            ...
                            if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
                                NpcService.gI().createTutorial(player, -1,
                                        "Chào mừng " + player.name + " đến với ngọc rồng V2\n"
    ...
    public void login2(MySession session, Message msg) {
        Service.gI().sendThongBaoOK(session, "Đăng Ký Tài Khoản Tại Trang Chủ Ngọc Rồng V2\nhttps://linhthuydanhbac.com");
    }
    ...
    private void sendThongBaoServer(Player player) {
        Service.gI().sendThongBaoFromAdmin(player, "Ra xã hội làm ăn bươn chải, liều thì ăn nhiều, không liều thì ăn ít. Muốn thành công thì phải chấp nhận trải qua đắng cay ngọt bùi. Làm ăn muốn kiếm được tiền phải chấp nhận mạo hiểm nguy hiểm một tí nhưng trong tầm kiểm soát. Xã hội này chỉ có làm, chịu khó, cần cù thì bù siêng năng, chỉ có làm thì mới có ăn, những cái loại không làm mà đòi có ăn thì ăn đầu buồi ăn cứt! Thế cho nó dễ! Xã hội này không làm thì chỉ có ăn cứt thôi. Nói thế cho nó nhanh, cho nó dễ hiểu.");
    }
}
