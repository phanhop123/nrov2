...
public class Mob {
    ...
    public List<ItemMap> getItemMobReward(Player player, int x, int yEnd) {
        ...
        //Set kich hoat
        if (((player.isActive() && Util.isTrue(1, 10000)) || (Manager.TEST && Util.isTrue(5, 100)) || Util.isTrue(1, 100000)) && MapService.gI().isMapUpSKH(mapid)) {
            short itTemp = (short) ItemService.gI().randTempItemKichHoat(player.gender);
            ItemMap it = new ItemMap(zone, itTemp, 1, x, yEnd, player.id);
            List<Item.ItemOption> ops = ItemService.gI().getListOptionItemShop(itTemp);
            if (!ops.isEmpty()) {
        ...
        if (MapService.gI().isMapCold(mapid)) {
            ...
            if (Util.isTrue(1, 20000) || (player.isActive() && Util.isTrue(1, 5000)) || (player.isAdmin() && Util.isTrue(10, 100))) {
            ...
            if ((Util.isTrue(1, 500) || (player.isActive() && Util.isTrue(1, 100))) && InventoryService.gI().fullSetThan(player)) {
